...
changelog:
----------
fixed case sensitive functionality
...
search_option_last_name = Radiobutton(mailerwindow, text="Search by Last Name", variable=search_by_var, value="Last Name", font=('', 12))
search_option_last_name.place(x=100, y=80)
search_option_customer_id = Radiobutton(mailerwindow, text="Search by Customer ID", variable=search_by_var, value="Customer ID", font=('', 12))
...
def search_name():
    search_term = search_entry.get().strip().upper()  # Convert search term to uppercase,to avoid mismatch
    connection = sqlite3.connect('crmdatabase200.db')
    cursor = connection.cursor()
    
    if search_by_var.get() == "Last Name":
        cursor.execute("SELECT email FROM customers WHERE UPPER(last_name) = ?", (search_term,))        
    ...
    recipient_label = Label(mailerwindow, text="EMAIL RECIPIENT:", font=('', 10))
    recipient_label.place(x=100, y=150)
    
    for i, result in enumerate(results):
        email = result[0]
        label = Label(mailerwindow, text=email, font=('', 10))
