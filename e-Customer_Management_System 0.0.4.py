'''
e-Customer_Management_System Version: 0.0.4
Date: 04/05/2024
...
def search(search_type, query_var, results_text):
    ...
    if search_type == "customers":
        ...
        columns = ("product_id", "product_name", "product_pieces", "product_price", "product_desc", "product_stock")
        table_name = "products"
    else:
...
def clearProductsView():
    ...
    product_query = "SELECT product_id, product_name, product_pieces, product_price, product_desc, product_stock FROM products"
...
def add_new_product():
    ...
    query = "INSERT INTO products(product_id, product_name, product_pieces, product_price, product_desc, product_stock) VALUES(?, ?, ?, ?, ?, ?)" 
    cursor.execute(query, (product_id, product_name, product_pieces, product_price, product_desc, product_stock))
    mydb.commit()
    clearProductsView()
...
def update_product():
    ...
    if messagebox.askyesno("UPDATE PRODUCT", "UPDATE PRODUCT ?"):
        query = "UPDATE products SET product_name = ?, product_pieces = ?, product_price = ? , product_desc = ? , product_stock = ? WHERE product_id = ?"
        cursor.execute(query, (product_name, product_pieces, product_price, product_desc, product_stock, product_id))
        mydb.commit()
        clearProductsView()
    else:
        return True
...
update_treeview(trv_products, "SELECT product_id, product_name, product_pieces, product_price, product_desc, product_stock FROM products")
...
lbl9 = ttk.Label(frame_products, text="Product_pieces")
