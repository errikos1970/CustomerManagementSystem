'''
e-Customer_Management_System Version: 2.0.7
Date: 08/06/2024
Author: Errikos Ntinos
Status: STABLE (??? VERIFY AND THEN UPLOAD)

changelog:
----------
* fixed some case sensitive functionality in email script - see mailer version 1.0.0
...
import subprocess # to run the emailer subscript

mainwindow = tk.Tk()
mainwindow.title("Customer Management System 2.0.7")
...
# the database functionality begin...

# Connect to SQLite database
mydb = sqlite3.connect('crmdatabase200.db')
cursor = mydb.cursor()

# Create customers table if it doesn't exist
cursor.execute('''CREATE TABLE IF NOT EXISTS customers
                  (customerid INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, age INTEGER, email TEXT, phone TEXT, address TEXT, registration_date DATE)''')
...
def search(search_type, query_var, results_text):
    query = query_var.get().upper()  # Convert the search query to uppercase TEST
    if not query:
        messagebox.showwarning("Empty Query", "Please enter a search query.")
        return

    if search_type == "customers":
        columns = ("customerid", "first_name", "last_name", "age", "email", "phone", "address")
    ...
    sql_query = f"SELECT {', '.join(columns)} FROM {table_name} WHERE {' OR '.join([f'{col} LIKE ?' for col in columns])}" #original code
    #sql_query = f"SELECT {', '.join(columns)} FROM {table_name} WHERE {' OR '.join([f'LOWER({col}) LIKE ?' for col in columns])}" #2.0.7
...
# Function to perform the search ΝΑ ΤΟ ΚΡΑΤΗΣΩ Η ΝΑ ΒΑΛΩ ΤΟ ΑΡΧΙΚΟ;
def search(search_type, query_var, results_text):
    query = query_var.get().upper()  # Convert the search query to uppercase
    if not query:
        messagebox.showwarning("Empty Query", "Please enter a search query.")
        return
    ...
    # Construct the SQL query dynamically
    sql_query = f"SELECT {', '.join(columns)} FROM {table_name} WHERE {' OR '.join([f'UPPER({col}) LIKE ?' for col in columns])}"
...
def clearCustomersView():
    ...
    customer_query = "SELECT customerid, first_name, last_name, age, email, phone, address FROM customers"
...
def get_customers_row(event):
    ...
    t7.set(item['values'][6])# for address 
    
def get_products_row(event):    
    item = trv_products.item(trv_products.focus())  # Use trv_products here
    t8.set(item['values'][0])# for product_id
    t9.set(item['values'][1])# for product_name
    t10.set(item['values'][2])# for stock
    t11.set(item['values'][3])# for product_price
    t12.set(item['values'][4])# for product_desc
...
def add_new_customer():
    ...
    query = "INSERT INTO customers(customerid, first_name, last_name, age, email, phone, address, registration_date) VALUES(NULL, ?, ?, ?, ?, ?, ?, DATE('now'))"
    cursor.execute(query, (fname, lname, age, email, phone, address))
    mydb.commit()
    clearCustomersView()

# added in 2.0.3 to clear the input fields
...
def update_customer():
    ...
    if messagebox.askyesno("UPDATE CUSTOMER", "UPDATE CUSTOMER ?"):
        query = "UPDATE customers SET first_name = ?, last_name = ?, age = ? , email = ? , phone = ? , address = ? WHERE customerid = ?" 
        cursor.execute(query, (fname, lname, age, email, phone, address, custid))
        mydb.commit()
        clearCustomersView()
        messagebox.showinfo("Update Successful", "Customer details have been successfully updated.")           
    else:
        return True
# added in 2.0.5 to clear the input fields TEST
    # Clear the input fields
    t1.set("")
    t2.set("")
    t3.set("")
...
def add_new_product():
    product_id = t8.get()
    product_name = t9.get().upper()
    stock = t10.get()
    product_price = t11.get()
    product_desc = t12.get().upper()
    # Check if any required field is empty
    if not product_id or not product_name or not stock or not product_price or not product_desc:
        messagebox.showerror("Error", "Please fill in all required fields.")
        return  
    
    # Validate stock and product_price to be numeric - added in 2.0.6 TEST
    if not stock.isdigit():
        messagebox.showerror("Input Error", "Stock must be an integer.")
        return
    if not product_price.replace('.', '', 1).isdigit():
        messagebox.showerror("Input Error", "Product price must be a number.")
        return  
...
def update_product():
    ...
    if not product_id:
        messagebox.showerror("Input Error", "Please select a product to update.")
        return

    # Check if all mandatory fields are provided
    if not product_name or not stock or not product_price:
        messagebox.showerror("Input Error", "Please provide Product Name, Stock, and Product Price.")
        return

    # Validate stock and product_price to be numeric
    if not stock.isdigit():
        messagebox.showerror("Input Error", "Stock must be an integer.")
        return
    ...
    if messagebox.askyesno("UPDATE PRODUCT", "UPDATE PRODUCT ?"):
        ...
        mydb.commit()
        clearProductsView()
        messagebox.showinfo("Update Successful", "Product details have been successfully updated.")       
    else:
        return True

# Function to create entry fields for the sale operation
def create_sale_entry_fields():
    global ent_sale_customer_id, ent_sale_product_id
# Call the function to create entry fields
create_sale_entry_fields()
...
def mail_me():
    # Run the mailer.py script asynchronously,to avoid stall of main script
    subprocess.Popen(["python", "mailer1.0.0.py"])
...
# added on 2.0.1
# the mail_me button
mail_me_menu = tk.Menu(menu, tearoff=False)
mail_me_menu.add_command(label='Send Email', command=mail_me)
menu.add_cascade(label='Send Email', menu=mail_me_menu)
...
q1 = tk.StringVar()
q2 = tk.StringVar()
t1 = tk.StringVar()# customerid
t2 = tk.StringVar()# first_name
t3 = tk.StringVar()# last_name
t4 = tk.StringVar()# age
t5 = tk.StringVar()# email
t6 = tk.StringVar()# phone
t7 = tk.StringVar() # address
################################
t8 = tk.StringVar()# product_id
t9 = tk.StringVar()# product_name
t10 = tk.StringVar()# stock
t11 = tk.StringVar()# product_price
t12 = tk.StringVar()# product_desc

# Define global variables for entry widgets
ent_sale_customer_id = None
ent_sale_product_id = None
ent_sale_quantity = tk.StringVar()  # Add this line in 2.0.6 DO WE NEED IT ???

# Create the customers Treeview
wrapper1 = ttk.LabelFrame(mainwindow, text="Customer List")
wrapper1.pack(fill="both", expand="yes", padx=20, pady=10)
trv_customers = ttk.Treeview(wrapper1, columns=(1, 2, 3, 4, 5, 6, 7), show="headings", height="5")
...
update_treeview(trv_customers, "SELECT customerid, first_name, last_name, age, email, phone, address FROM customers")
...
trv_customers.heading(7, text="Address")
...
trv_customers.column('#7', width=250, minwidth=200)# change to width=150, minwidth=100)
...
lbl7 = ttk.Label(frame_customers, text="Address:")
lbl7.grid(row=6, column=0, padx=5, pady=5, sticky=tk.W)
...
ent7 = ttk.Entry(frame_customers, textvariable=t7)
ent7.grid(row=6, column=1, padx=5, pady=5, sticky=tk.W)
...
add_btn.grid(row=7, column=0, columnspan=2, padx=5, pady=5, sticky=tk.EW)

update_btn = ttk.Button(frame_customers, text="Update Customer", command=update_customer)
update_btn.grid(row=8, column=0, columnspan=2, padx=5, pady=5, sticky=tk.EW)

delete_btn = ttk.Button(frame_customers, text="Delete Customer", command=delete_customer)
delete_btn.grid(row=9, column=0, columnspan=2, padx=5, pady=5, sticky=tk.EW)
...
# products labels 
lbl8 = ttk.Label(frame_products, text="Product ID")
lbl8.grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)

lbl9 = ttk.Label(frame_products, text="Product Name")
lbl9.grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)

lbl10 = ttk.Label(frame_products, text="Product Stock")
lbl10.grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)

lbl11 = ttk.Label(frame_products, text="Product Price")
lbl11.grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)

lbl12 = ttk.Label(frame_products, text="Product Description")
lbl12.grid(row=4, column=0, padx=5, pady=5, sticky=tk.W)

# Adjust grid positions for products entries
ent8 = ttk.Entry(frame_products, textvariable=t8)
ent8.grid(row=0, column=1, padx=5, pady=5, sticky=tk.W)

ent9 = ttk.Entry(frame_products, textvariable=t9)
ent9.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)

ent10 = ttk.Entry(frame_products, textvariable=t10)
ent10.grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)

ent11 = ttk.Entry(frame_products, textvariable=t11)
ent11.grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)

ent12 = ttk.Entry(frame_products, textvariable=t12)
ent12.grid(row=4, column=1, padx=5, pady=5, sticky=tk.W)
...
add_product_btn.grid(row=7, column=0, columnspan=2, padx=5, pady=5, sticky=tk.EW)

update_product_btn = ttk.Button(frame_products, text="Update Product", command=update_product)
update_product_btn.grid(row=8, column=0, columnspan=2, padx=5, pady=5, sticky=tk.EW)

delete_product_btn = ttk.Button(frame_products, text="Delete Product", command=delete_product)
delete_product_btn.grid(row=9, column=0, columnspan=2, padx=5, pady=5, sticky=tk.EW)

# Create a frame for the sales labels, entries, and buttons
frame_sales = ttk.Frame(mainwindow)
frame_sales.pack(side=tk.LEFT, padx=20, pady=10, anchor=tk.NW)  # Align anchor=tk.NW) 

# Create a label for "sales Data" and place it above the frame
#lbl_sales = ttk.Label(mainwindow, text="Sales Data")
#lbl_sales.pack(side=tk.TOP, padx=20, pady=10, anchor=tk.NW)  # Align to 
...
lbl_sale_customer_id = ttk.Label(frame_sales, text="Customer ID for Sale:")
lbl_sale_customer_id.grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)

ent_sale_customer_id = ttk.Entry(frame_sales)
ent_sale_customer_id.grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)

lbl_sale_product_id = ttk.Label(frame_sales, text="Product ID for Sale:")
lbl_sale_product_id.grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)

ent_sale_product_id = ttk.Entry(frame_sales)
ent_sale_product_id.grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)

# Add entry field for quantity
lbl_sale_quantity = ttk.Label(frame_sales, text="Quantity for Sale:")
lbl_sale_quantity.grid(row=4, column=0, padx=5, pady=5, sticky=tk.W)

ent_sale_quantity = ttk.Entry(frame_sales)
ent_sale_quantity.grid(row=5, column=0, padx=5, pady=5, sticky=tk.W)

btn_sale_product = ttk.Button(frame_sales, text="Sell Product", command=sell_product)
btn_sale_product.grid(row=6, column=0, padx=5, pady=5, sticky=tk.EW)
